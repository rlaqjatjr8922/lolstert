import time
import subprocess
from pathlib import Path

import cv2
import numpy as np
import mss
import win32gui
import win32con

from src.match.slot_turn_detector import detect_turn_slot, draw_turn_debug


# =========================
# 설정
# =========================
SCRCPY_EXE = r"C:\scrcpy-win64-v3.3.3\scrcpy.exe"
WINDOW_WAIT_SECONDS = 12
CAPTURE_FPS = 2
RECOMMEND_COOLDOWN = 2.0

DEBUG_DIR = Path(r"C:\Users\gimbe\OneDrive\Desktop\lol_project\밴픽\dataset\raw_screens\pregame")
DEBUG_DIR.mkdir(parents=True, exist_ok=True)

LATEST_FRAME_PATH = DEBUG_DIR / "realtime_latest.png"


def launch_scrcpy():
    try:
        subprocess.Popen(
            [SCRCPY_EXE],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        print(f"[INFO] scrcpy 실행 요청: {SCRCPY_EXE}")
    except FileNotFoundError:
        print(f"[오류] scrcpy 실행 파일을 찾을 수 없습니다: {SCRCPY_EXE}")


def save_image_korean_path(path, image):
    ext = Path(path).suffix
    ok, buf = cv2.imencode(ext, image)
    if not ok:
        return False
    buf.tofile(str(path))
    return True


def is_probable_scrcpy_mirror_window(title: str) -> bool:
    t = title.strip().lower()
    if not t:
        return False

    positive = [
        "scrcpy",
        "android",
        "sm-",
        "pixel",
        "xiaomi",
        "redmi",
        "galaxy",
        "device",
    ]
    if not any(p in t for p in positive):
        return False

    negative = [
        "scrcpy-win64",
        "검색",
        "file explorer",
        "explorer",
        "local disk",
        "로컬 디스크",
    ]
    if any(n in t for n in negative):
        return False

    return True


def debug_print_visible_windows():
    print("\n[DEBUG] 현재 보이는 창 제목들:")
    titles = []

    def enum_handler(hwnd, _ctx):
        if not win32gui.IsWindowVisible(hwnd):
            return
        title = win32gui.GetWindowText(hwnd).strip()
        if title:
            titles.append(title)

    win32gui.EnumWindows(enum_handler, None)

    for t in titles:
        print(" -", t)


def find_scrcpy_window():
    result = []

    def enum_handler(hwnd, _ctx):
        if not win32gui.IsWindowVisible(hwnd):
            return

        title = win32gui.GetWindowText(hwnd).strip()
        if not title:
            return

        if is_probable_scrcpy_mirror_window(title):
            result.append((hwnd, title))

    win32gui.EnumWindows(enum_handler, None)

    if not result:
        return None, None

    for hwnd, title in result:
        if "scrcpy" in title.lower() and "win64" not in title.lower():
            return hwnd, title

    return result[0]


def wait_for_window(timeout=12):
    start = time.time()

    while time.time() - start < timeout:
        hwnd, title = find_scrcpy_window()
        if hwnd is not None:
            return hwnd, title
        time.sleep(0.3)

    return None, None


def get_client_rect_on_screen(hwnd):
    left, top, right, bottom = win32gui.GetClientRect(hwnd)
    pt_left_top = win32gui.ClientToScreen(hwnd, (left, top))
    pt_right_bottom = win32gui.ClientToScreen(hwnd, (right, bottom))

    x1, y1 = pt_left_top
    x2, y2 = pt_right_bottom

    return {
        "left": x1,
        "top": y1,
        "width": x2 - x1,
        "height": y2 - y1,
    }


def bring_window_to_front(hwnd):
    try:
        win32gui.ShowWindow(hwnd, win32con.SW_RESTORE)
        win32gui.SetForegroundWindow(hwnd)
    except Exception:
        pass


def capture_window_frame(sct, hwnd):
    rect = get_client_rect_on_screen(hwnd)

    if rect["width"] <= 0 or rect["height"] <= 0:
        return None

    shot = sct.grab(rect)
    frame = np.array(shot)
    frame = cv2.cvtColor(frame, cv2.COLOR_BGRA2BGR)
    return frame


def should_run_recommend(turn_info):
    return turn_info["is_ally_pick_turn"] and turn_info["is_my_turn"]


def main():
    print(f"[INFO] 저장 폴더: {DEBUG_DIR}")
    print(f"[INFO] 저장 파일: {LATEST_FRAME_PATH}")

    launch_scrcpy()

    hwnd, title = wait_for_window(WINDOW_WAIT_SECONDS)
    if hwnd is None:
        print("[오류] scrcpy 미러링 창을 찾지 못했습니다.")
        debug_print_visible_windows()
        return

    print(f"[INFO] 연결된 창: {title}")
    bring_window_to_front(hwnd)

    frame_interval = 1.0 / CAPTURE_FPS
    last_recommend_time = 0.0
    last_save_time = 0.0

    with mss.mss() as sct:
        while True:
            t0 = time.time()

            frame = capture_window_frame(sct, hwnd)
            if frame is None:
                print("[오류] 프레임 캡처 실패")
                time.sleep(0.5)
                continue

            turn_info = detect_turn_slot(frame)
            debug_frame = draw_turn_debug(frame, turn_info)

            run_recommend = should_run_recommend(turn_info)

            if run_recommend:
                now = time.time()
                if now - last_recommend_time >= RECOMMEND_COOLDOWN:
                    print("\n=== 추천 로직 실행 조건 만족 ===")
                    print(f"ally_active_slots = {turn_info['ally_active_slots']}")
                    print(f"enemy_active_slots = {turn_info['enemy_active_slots']}")
                    print(f"ally_pattern = {turn_info['ally_pattern']}")
                    print(f"enemy_pattern = {turn_info['enemy_pattern']}")
                    print(f"pick_turn_team = {turn_info['pick_turn_team']}")
                    print(f"is_my_turn = {turn_info['is_my_turn']}")
                    print(">>> 추천 로직 실행 자리 <<<")
                    last_recommend_time = now

            now = time.time()
            if now - last_save_time >= 0.5:
                ok = save_image_korean_path(LATEST_FRAME_PATH, debug_frame)
                print(f"[SAVE] {LATEST_FRAME_PATH} / ok={ok}")
                last_save_time = now

            elapsed = time.time() - t0
            sleep_time = frame_interval - elapsed
            if sleep_time > 0:
                time.sleep(sleep_time)


if __name__ == "__main__":
    main()