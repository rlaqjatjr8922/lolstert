...
def detect_turn_slot(img):
    ally_blue = _segment_color_presence(img, ALLY_TURN_BAR_ROI, [(BLUE_LOWER, BLUE_UPPER)])
    ally_yellow = _segment_color_presence(img, ALLY_TURN_BAR_ROI, [(YELLOW_LOWER, YELLOW_UPPER)])
    enemy_red = _segment_color_presence(img, ENEMY_TURN_BAR_ROI, [(RED1_LOWER, RED1_UPPER), (RED2_LOWER, RED2_UPPER)])
    ...
    pick_info = detect_pick_turn_from_patterns(ally_active_slots, enemy_active_slots)
...
def draw_turn_debug(img, turn_info):
    ...
    cv2.putText(out, "ALLY ROI", (ax1 + 8, max(20, ay1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)

    cv2.rectangle(out, (ex1, ey1), (ex2, ey2), (200, 200, 200), 2)
    cv2.putText(out, "ENEMY ROI", (ex1 - 100, max(20, ey1 - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200, 200, 200), 1, cv2.LINE_AA)

    for idx, _abs_y1, _abs_y2, cy in turn_info["ally_slot_ranges"]:
        cv2.line(out, (ax1 - 20, cy), (ax2 + 160, cy), (180, 180, 180), 1)
        cv2.putText(out, f"S{idx}", (ax2 + 8, cy + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)

    for idx, _abs_y1, _abs_y2, cy in turn_info["enemy_slot_ranges"]:
        cv2.line(out, (ex1 - 160, cy), (ex2 + 20, cy), (180, 180, 180), 1)
        cv2.putText(out, f"S{idx}", (ex1 - 35, cy + 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
    ...
    for s in turn_info["blue_slots"]:
        ...
        cv2.putText(out, f"BLUE S{s} r={ratio:.2f}", (ax1 + 12, max(20, cy - 8)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (255, 0, 0), 1, cv2.LINE_AA)
    ...
    for s in turn_info["yellow_slots"]:
        ...
        cv2.putText(out, f"YELLOW S{s} r={ratio:.2f}", (ax1 + 12, min(h - 10, cy + 18)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0, 255, 255), 1, cv2.LINE_AA)
    ...
    for s in turn_info["red_slots"]:
        ...
        cv2.putText(out, f"RED S{s} r={ratio:.2f}", (max(10, ex1 - 270), min(h - 10, cy + 18)), cv2.FONT_HERSHEY_SIMPLEX, 0.48, (0, 0, 255), 1, cv2.LINE_AA)
    ...
    if turn_info["is_my_turn"]:
        cv2.putText(out, "MY TURN (YELLOW DETECTED)", (30, 200), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 255), 2, cv2.LINE_AA)
